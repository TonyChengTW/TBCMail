...
if ($type eq 'ip') {
    if ($maillog_file =~ /gz/) {
       #system "gunzip -c $maillog_file|egrep -v 'gethostbyaddr|localhost|/dev/null|cucipop|127.0.0.1|reject|mailer=local'|egrep '\[.*\..*\..*\..*\]' >/disk_f720/tmp.txt";
       system "gunzip -c $maillog_file|grep 'from=<' > /disk_f720/tmp.txt";
    } else {
       ...
       system "cat $maillog_file|grep 'from=<' > /disk_f720/tmp.txt";
    }
